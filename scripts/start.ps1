...
.DESCRIPTION
    ...
    Does NOT do first-time setup (venv creation, npm install, .env) -- see
    RUNNING.md's "One-time setup". This script fails loud and early if
    that hasn't happened yet, rather than trying to paper over it.

    DOES auto-bootstrap the two things needed to reach a populated "running
    demo" state with zero extra manual steps: curriculum ingestion (only
    when data\chroma is empty -- a real network/API cost, so not repeated
    on every start) and demo-student seeding (every start -- offline, no
    LLM calls, idempotent, so just always making sure it's done is simpler
    than detecting whether it's needed). Both degrade to a warning rather
    than blocking startup if they fail.
...
# Same reasoning as $VenvPython above: prefer whatever resolves on PATH,
# but fall back to the well-known winget install location. A PowerShell
# process started before Node was installed (or before a PATH change
# propagated) won't see an updated machine PATH until it's restarted, so
# `Get-Command npm` can fail here even on a machine where npm works fine
# in a fresh terminal -- see RUNNING.md's one-time setup note about this.
$NpmCmd = "npm.cmd"
$onPath = Get-Command npm.cmd -ErrorAction SilentlyContinue
if ($onPath) {
    $NpmCmd = $onPath.Source
} else {
    $fallback = "C:\Program Files\nodejs\npm.cmd"
    if (Test-Path $fallback) {
        $NpmCmd = $fallback
    } else {
        Write-Host "'npm.cmd' not found on PATH and not at the default winget install location." -ForegroundColor Red
        Write-Host "Install Node.js (see RUNNING.md's one-time setup), then open a fresh terminal and re-run start.bat." -ForegroundColor Red
        exit 1
    }
}

$ChromaDir = Join-Path $RepoRoot "data\chroma"
$ChromaEmpty = (-not (Test-Path $ChromaDir)) -or ((Get-ChildItem $ChromaDir -ErrorAction SilentlyContinue | Measure-Object).Count -eq 0)
if ($ChromaEmpty) {
    Write-Host "First run: no curriculum ingested yet -- fetching from Isaac Science / Ada Computer Science (one-time, ~1-2 min, needs network)..." -ForegroundColor Cyan
    foreach ($src in @("isaac_science", "isaac_chemistry", "ada_computer_science")) {
        Write-Host "  ingesting $src..."
        & $VenvPython -m stage3.ingest --source $src
        if ($LASTEXITCODE -ne 0) {
            Write-Host "  Warning: ingest of '$src' failed (exit $LASTEXITCODE)." -ForegroundColor Yellow
            Write-Host "  Retrieval for that source will return nothing until you re-run: python -m stage3.ingest --source $src" -ForegroundColor Yellow
        }
    }
}

# ---------------------------------------------------------------------------
# Seed demo students -- offline, no LLM calls, idempotent (skips topics
# already seeded), so just always doing it is simpler than detecting
# whether it's needed. This is what gets the student dropdown and mastery
# bars populated with zero manual steps.
# ---------------------------------------------------------------------------
Write-Host "Seeding demo students (offline, idempotent)..."
& $VenvPython (Join-Path $RepoRoot "scripts\seed_demo_students.py")
if ($LASTEXITCODE -ne 0) {
    Write-Host "Warning: demo-student seeding failed (exit $LASTEXITCODE) -- continuing without demo data." -ForegroundColor Yellow
}
