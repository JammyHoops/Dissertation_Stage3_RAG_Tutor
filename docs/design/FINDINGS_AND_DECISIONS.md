...
**Update rule (decided 2026-08-13)**: EWMA —
...
**Mastery is diagnostic-seeded, not continuously re-graded**: a short
LLM-graded Q&A opens each topic thread and updates
...
actually got wired instead. Called out explicitly as a genuine
architecture fork (note injected only at diagnostic-opening vs. also
re-injected into early normal turns) rather than silently picked — every
LLM call here is
...
A later refinement improved on the original design: rather than only
...
synthetic fixture from the real Stage 1 pipeline: `flag_status`
...
this project's evaluation is a qualitative expert review
...
**Cold-start prior**: uniform `Beta(1,1)`, chosen deliberately rather
...
**Reviewer interface is CSV export/import**, chosen deliberately
